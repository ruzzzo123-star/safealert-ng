      ...
      fetch(request)
        ...
        .catch(() => {
          return caches.match(request).then((cachedResponse) => {
            return cachedResponse || caches.match('/safealert-pwa.html');
          });
        })
    );
    return;
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    ...
    data: {
      url: data.url || '/safealert-pwa.html',
  ...
  if (data.type === 'emergency') {
    ...
    options.actions = [
      { action: 'respond', title: '🚨 I\'m Safe', icon: '/icons/safe.png' },
...
self.addEventListener('notificationclick', (event) => {
  ...
  const urlToOpen = event.notification.data?.url || '/safealert-pwa.html';
      ...
      clients.matchAll({ type: 'window' })
        .then((clientList) => {
          ...
          if (clientList.length === 0 && clients.openWindow) {
            return clients.openWindow('/safealert-pwa.html?action=sos');
          }
        })
    );
  }
